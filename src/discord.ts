export class DiscordClient {
	...
	public connect() {
		...
		try {
			// Discord local RPC usually runs on ports 6463-6472
			this.socket = new WebSocket(`ws://127.0.0.1:6463/?v=1&client_id=${this.clientId}`);

			this.socket.onopen = () => {
				console.log('Connected to Discord RPC');
				this.connected = true;

				// If there's an activity waiting to be sent, send it now
			...
			args: {
				pid:
					typeof process !== 'undefined' && process.pid
						? process.pid
						: Math.floor(Math.random() * 10000), // Pid is usually required by Discord RPC
				activity: activity,
			},
	...
	public UpdateRichPresence(options: {
		...
		if (!this.connected || !this.socket) {
			// Queue it up to be sent once connected
